...
@WebServlet(name = "SvExportarExcelContenedor", urlPatterns = {"/SvExportarExcelContenedor"})
public class SvExportarExcelContenedor extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
       ...
       // Recibir parámetros de filtrado
        ...
        List<RegistroContenedores> registrosFiltrados = obtenerRegistrosFiltrados(fechaInicio, fechaFin, request);
        ...
        try (XSSFWorkbook workbook = new XSSFWorkbook()) { // Usar XSSFWorkbook para .xlsx
            ...
            headerRow.createCell(2).setCellValue("Contenedor");
            ...
            int rowIndex = 1;
            for (RegistroContenedores registro : registrosFiltrados) {
                Row row = sheet.createRow(rowIndex++);
                row.createCell(0).setCellValue(registro.getFecha());
                row.createCell(1).setCellValue(registro.getMaquina()); // Asumiendo que registro.getFecha() devuelve un String con formato correcto
                row.createCell(2).setCellValue(registro.getContenedor());
                row.createCell(3).setCellValue(registro.getMedida());
                row.createCell(4).setCellValue(registro.getMovimiento());
                row.createCell(5).setCellValue(registro.getComentarios());
                row.createCell(6).setCellValue(registro.getOperador());
            }
    ...
    private List<RegistroContenedores> obtenerRegistrosFiltrados(String fechaInicio, String fechaFin, HttpServletRequest request) {
        List<RegistroContenedores> listaRegistros = (List<RegistroContenedores>) request.getSession().getAttribute("listaRegistros");
